                ...
                <div className='basis-5/6 relative'>
                    <div className='flex absolute top-0 left-0 right-0 items-center justify-between'>
                        <Link to={`/product/${prop?.data?.slug}`}>
                            <h2 className='hover:text-blue-500 dark:text-white cursor-pointer w-4/5 text-sm md:text-base line-clamp-1'>
                                <span className='font-bold text-red-500'>[DEAL] </span>
                                {prop?.data?.title}
                            </h2>
                        </Link>
                        <i onClick={() => removeCart(prop?.data?._id)} className="hover:text-red-600 dark:text-purple-500 text-lg cursor-pointer fa-regular fa-trash-can"></i>
                    </div>
                    <div className='flex absolute bottom-0 left-0 right-0 items-end justify-between'>
                        ...
                        <div className='text-stone-600 flex flex-col items-end'>
                            <div className='flex items-center text-sm md:text-xl'>
                                <Button onClick={() => decrease(prop?.data?._id)} className='rounded-none px-1 md:px-3 dark:bg-black dark:text-white dark:border-slate-700'>
                                    <i className="fa-solid fa-minus"></i>
                                </Button>
                                <input htmltype='text' className='dark:bg-black bg-stone-100 dark:text-white w-5 md:w-14 text-center h-8 border-solid' disabled value={prop?.data?.quanlity} />
                                <Button onClick={() => increase(prop?.data?._id)} className='rounded-none px-1 md:px-3 dark:bg-black dark:text-white dark:border-slate-700'>
